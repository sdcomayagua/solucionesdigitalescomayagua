(() => {
  ...
  function bindEvents(){
    ...
    document.addEventListener('input', (event) => {
      ...
      if (el.matches('[data-quote-field]')) {
        ...
        if (el.dataset.quoteField === 'zone' || el.dataset.quoteField === 'localShipping') render();
    ...
    document.addEventListener('change', (event) => {
      ...
      if (el.matches('[data-quote-field]')) { state.quoteInfo[el.dataset.quoteField] = el.value; if (el.dataset.quoteField === 'zone' || el.dataset.quoteField === 'localShipping') render(); else renderSoftQuoteMessage(); }
  ...
  function renderQuoteSummaryTotals(){
    const t = quoteTotals();
    if (!t.outside) return `<div class="total-line"><span>Productos</span><span>${lempira(t.subtotal)}</span></div>${t.localShipping > 0 ? `<div class="total-line"><span>Envío local</span><span>${lempira(t.localShipping)}</span></div>` : ''}<div class="total-line big"><span>Total Comayagua</span><span>${lempira(t.localTotal)}</span></div>`;
  ...
  function buildQuoteMessage(mode = quoteDefaultMode()){
    ...
    if (!items.length) {
      ...
      if (!outsideMode) {
        if (totals.localShipping > 0) {
          ...
          lines.push('');
          lines.push(`✅ *Total a pagar en Comayagua:* ${lempira(totals.localTotal)}`);
        } else {
          lines.push('');
          lines.push(`✅ *Total productos en Comayagua:* ${lempira(totals.subtotal)}`);
        }
      }
    ...
    lines.push('');
    lines.push('ℹ️ Precio sujeto al producto y al tipo de envío cotizado.');
    return lines.join('\n');
  }

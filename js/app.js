(() => {
  ...
  function bindEvents(){
    ...
    document.addEventListener('change', (event) => {
      ...
      if (el.matches('[data-quote-field]')) { state.quoteInfo[el.dataset.quoteField] = el.value; if (el.dataset.quoteField === 'zone') render(); else renderSoftQuoteMessage(); }
  ...
  function renderQuoteFields(){
    ...
    return `<div class="form-grid quote-location-grid">
      ...
      <div style="grid-column:1/-1">
        ...
        <textarea class="textarea" data-quote-field="note" placeholder="Ejemplo: confirmar disponibilidad antes de pagar...">${esc(q.note || '')}</textarea>
      </div>
    </div>`;
  }
  ...
  function renderQuoteDeliveryPanel(){
    const t = quoteTotals();
    if (!t.outside) {
      return `<div class="panel shipping-panel local-panel">
        <div class="panel-header">
          <div>
            <h2 class="panel-title">Cliente en Comayagua</h2>
            <p class="panel-subtitle">No se suma envío. Solo se muestra el total de productos.</p>
          </div>
          <span class="badge ok">Sin envío</span>
        </div>
        <div class="local-total-box">
          <span>Total a cobrar</span>
          <b>${lempira(t.subtotal)}</b>
        </div>
        <p class="shipping-note">Este total aplica cuando la entrega es dentro de Comayagua.</p>
      </div>`;
    }
    return `<div class="panel shipping-panel">
      <div class="panel-header">
        <div>
  ...
  function renderQuoteSummaryTotals(){
    const t = quoteTotals();
    if (!t.outside) return `<div class="total-line big"><span>Total Comayagua</span><span>${lempira(t.subtotal)}</span></div>`;
  ...
  function buildQuoteMessage(){
    ...
    lines.push('');
    lines.push(`📍 Ubicación: ${totals.outside ? 'Fuera de Comayagua' : 'Comayagua'}`);
    lines.push('');
    if (!items.length) {
      ...
      if (totals.outside) {
        ...
        lines.push('');
        lines.push('🚚 *Opciones de pago y envío:*');
        lines.push(`🏦 Depósito / envío normal: *${lempira(totals.normalTotal)}*`);
        lines.push(`   Productos + envío ${lempira(totals.shipping)}.`);
        lines.push(`📦 Pagar al recibir: *${lempira(totals.codTotal)}*`);
        lines.push(`   Base ${lempira(totals.commissionBase)} + 10% comisión (${lempira(totals.commission)}).`);
      } else {
        lines.push(`✅ *Total a pagar en Comayagua: ${lempira(totals.subtotal)}*`);
        lines.push('Sin costo de envío dentro de Comayagua.');
      }
    }
    if (info.note) {
      lines.push('');
  ...
  function quoteTotals(){
    ...
    const outside = isOutsideComayagua();

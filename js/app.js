  ...
  function handleAction(action, el){
    ...
    switch(action){
      ...
      case 'detail-whatsapp':
        openWhatsApp(buildSingleProductMessage(id));
        break;
      case 'close-modal':
        state.modal = null;
        render();
      ...
      case 'copy-quote':
        ...
        break;
      case 'open-whatsapp':
        openWhatsApp(buildQuoteMessage(el.dataset.mode || quoteDefaultMode()));
        break;
      case 'confirm-sale':
        confirmSale();
        break;
      ...
      <section class="cart-layout quote-layout">
        ...
        <aside class="panel summary-box">
          ...
          <div class="whatsapp-actions mt-16">
            <button class="btn green full whatsapp-btn" data-action="open-whatsapp" data-mode="comayagua">📲 WhatsApp Comayagua <small>No cobra envío</small></button>
            <button class="btn primary full whatsapp-btn" data-action="open-whatsapp" data-mode="normal">🚚 WhatsApp envío normal <small>Productos + Lps. 110</small></button>
            <button class="btn amber full whatsapp-btn" data-action="open-whatsapp" data-mode="cod">📦 WhatsApp pagar al recibir <small>Productos + envío + 10%</small></button>
            <button class="btn blue full whatsapp-btn" data-action="open-whatsapp" data-mode="both">📲 WhatsApp ambos <small>Muestra las 2 opciones</small></button>
    ...
    return `<div class="modal-backdrop" data-action="close-modal">
      <section class="modal" onclick="event.stopPropagation()" role="dialog" aria-modal="true">
        ...
        <div class="detail-action-grid mt-16">
          ...
          <button class="btn blue full" data-action="detail-whatsapp" data-id="${escAttr(p.id)}">📲 WHATSAPP</button>
        </div>
  ...
  function buildQuoteMessage(mode = quoteDefaultMode()){
    ...
    if (!items.length) {
      ...
      if (!outsideMode) {
        ...
        lines.push('🚫 No se cobra envío dentro de Comayagua.');
      }

      if (cleanMode === 'normal') {
        lines.push(`🚚 *Envío fuera de Comayagua:* ${lempira(totals.shipping)}`);
        lines.push('');
        lines.push(`✅ *Total envío normal:* ${lempira(totals.normalTotal)}`);
      ...
      if (cleanMode === 'cod') {
        ...
        lines.push('');
        lines.push(`✅ *Total pagar al recibir:* ${lempira(totals.codTotal)}`);
      ...
      if (cleanMode === 'both') {
        ...
        lines.push(`1️⃣ *Envío normal / depósito:* ${lempira(totals.normalTotal)}`);
        lines.push('   • Productos + envío.');
        lines.push('');
        lines.push(`2️⃣ *Pagar al recibir:* ${lempira(totals.codTotal)}`);
        lines.push(`   • Productos + envío = ${lempira(totals.commissionBase)}`);
        lines.push(`   • Comisión 10% = ${lempira(totals.commission)}`);
      }
    }

(() => {
  ...
  const state = {
    ...
    status: 'todos',
    quoteInfo: {
      customer: '', phone: '', zone: 'Comayagua', delivery: 'Envío normal', note: '', payment: 'Efectivo / transferencia'
    },
    saleInfo: {
  ...
  async function init(){
    ...
    try {
      const data = Array.isArray(window.SDC_PRODUCTS) ? window.SDC_PRODUCTS : await fetchInitialProducts();
  ...
  function bindEvents(){
    ...
    document.addEventListener('change', (event) => {
      ...
      if (el.matches('[data-quote-field]')) { state.quoteInfo[el.dataset.quoteField] = el.value; renderSoftQuoteMessage(); }
  ...
  function handleAction(action, el){
    ...
    switch(action){
      ...
      case 'close-modal':
        ...
        break;
      case 'copy-quote':
        copyText(buildQuoteMessage(), 'Cotización copiada.');
        break;
      case 'open-whatsapp':
  ...
  function renderTopActions(){
    ...
    if (state.route === 'quote') return `<button class="btn primary" data-action="open-whatsapp">📲 WhatsApp</button><button class="btn" data-action="copy-quote">Copiar</button>`;
    if (state.route === 'sell') return `<button class="btn green" data-action="confirm-sale">Confirmar venta</button>`;
    if (state.route === 'admin') return `<button class="btn primary" data-action="admin-new">＋ Producto</button><button class="btn" data-action="export-products">Exportar</button>`;
    ...
    return `
      <section class="hero">
        ...
        <div class="grid cols-2">
          ...
          ${stat('Unidades', m.units, '🧮', 'Disponibles en inventario')}
          ${stat('Ventas del día', lempira(m.todaySales), '💵', `${m.todayReceipts} recibos hoy`)}
          ${stat('Ganancia estimada', lempira(m.profit), '📈', 'Según costo y precio')}
        </div>
      </section>
      <section class="grid cols-4 mt-16">
        ${stat('Valor venta total', lempira(m.saleValue), '🏷️', 'Precio x stock')}
        ${stat('Inversión total', lempira(m.investment), '🧾', 'Costo x stock')}
        ${stat('Stock bajo', m.lowStock, '⚠️', 'Productos con 5 o menos')}
        ${stat('Agotados', m.outStock, '⛔', 'Productos sin existencia')}
      </section>
  ...
  function renderProductCard(p){
    return `
      <article class="product-card">
        <button class="product-media" data-action="detail" data-id="${escAttr(p.id)}" aria-label="Ver detalle de ${escAttr(p.name)}">
        ...
        <div class="product-body">
          ...
          <div class="product-actions">
            <button class="btn" data-action="detail" data-id="${escAttr(p.id)}">Detalle</button>
            <button class="btn primary" data-action="add-cart" data-cart="quote" data-id="${escAttr(p.id)}">Cotizar</button>
            <button class="btn green" data-action="add-cart" data-cart="sale" data-id="${escAttr(p.id)}" ${canSell(p) ? '' : 'disabled'}>Vender</button>
            <button class="btn blue" data-action="add-cart" data-cart="quote" data-id="${escAttr(p.id)}">WhatsApp</button>
          </div>
        </div>
      </article>
    `;
    ...
    return `
      <section class="cart-layout">
        <div class="grid">
          ...
          <div class="panel">
            <div class="panel-header"><div><h2 class="panel-title">Datos del cliente</h2><p class="panel-subtitle">Estos datos salen en el mensaje de WhatsApp.</p></div></div>
            ${renderQuoteFields()}
          </div>
        </div>
        <aside class="panel summary-box">
          <div class="panel-header"><div><h2 class="panel-title">Mensaje listo</h2><p class="panel-subtitle">Copia o envía directo a WhatsApp.</p></div></div>
          <div class="message-preview" id="quoteMessage">${esc(buildQuoteMessage())}</div>
          <div class="total-line big"><span>Total</span><span>${lempira(cartTotal('quote'))}</span></div>
          <div class="btn-row mt-16"><button class="btn primary full" data-action="open-whatsapp">📲 Enviar por WhatsApp</button><button class="btn full" data-action="copy-quote">Copiar mensaje</button></div>
        </aside>
      </section>
    `;
  }
  ...
  function renderQuoteFields(){
    ...
    return `<div class="form-grid">
      ${inputField('Cliente', 'customer', q.customer, 'quote', 'Nombre del cliente')}
      ${inputField('Teléfono', 'phone', q.phone, 'quote', 'Opcional')}
      ${inputField('Zona / municipio', 'zone', q.zone, 'quote', 'Comayagua')}
      <div><label class="label">Tipo de entrega</label><select class="select" data-quote-field="delivery">${['Envío normal','Pagar al recibir','Entrega local','Pasar a recoger'].map(v => `<option ${q.delivery === v ? 'selected' : ''}>${v}</option>`).join('')}</select></div>
      <div><label class="label">Forma de pago</label><select class="select" data-quote-field="payment">${['Efectivo / transferencia','Efectivo','Transferencia','Pendiente de confirmar'].map(v => `<option ${q.payment === v ? 'selected' : ''}>${v}</option>`).join('')}</select></div>
      <div style="grid-column:1/-1"><label class="label">Nota adicional</label><textarea class="textarea" data-quote-field="note" placeholder="Ejemplo: No incluye envío, confirmar disponibilidad antes de pagar...">${esc(q.note)}</textarea></div>
    </div>`;
  }
  ...
  function renderAdmin(){
    ...
    return `<section class="panel">
      <div class="panel-header"><div><h2 class="panel-title">Administrar productos</h2><p class="panel-subtitle">Edita precios, costos, stock, categorías e imágenes. Los cambios quedan en este navegador.</p></div>
        <div class="btn-row"><button class="btn primary" data-action="admin-new">＋ Producto</button><button class="btn" data-action="import-products">Importar</button><button class="btn" data-action="export-products">Exportar</button><button class="btn red" data-action="reset-products">Restaurar</button></div>
      </div>
      ${renderToolbar()}
  ...
  function renderDetailModal(p){
    return `<div class="modal-backdrop" data-action="close-modal">
      <section class="modal" onclick="event.stopPropagation()" role="dialog" aria-modal="true">
        ...
        <div class="btn-row mt-16"><button class="btn primary" data-action="add-cart" data-cart="quote" data-id="${escAttr(p.id)}">Agregar a cotización</button><button class="btn green" data-action="add-cart" data-cart="sale" data-id="${escAttr(p.id)}" ${canSell(p) ? '' : 'disabled'}>Agregar a venta</button><button class="btn" data-action="admin-edit" data-id="${escAttr(p.id)}">Editar</button></div>
      </section>
    </div>`;
  }
  ...
  function buildQuoteMessage(){
    ...
    lines.push('');
    if (info.customer) lines.push(`👤 Cliente: ${info.customer}`);
    if (info.phone) lines.push(`📞 Teléfono: ${info.phone}`);
    lines.push(`📍 Zona: ${info.zone || 'Comayagua'}`);
    lines.push(`🚚 Entrega: ${info.delivery}`);
    lines.push('');
    if (!items.length) {
      ...
      items.forEach((item, index) => {
        ...
        lines.push(`   Cantidad: ${qty} · Precio: ${lempira(p.price)} · Subtotal: ${lempira(p.price * qty)}`);
      });
      lines.push('');
      lines.push(`💰 *Total: ${lempira(cartTotal('quote'))}*`);
    }
    lines.push('');
    lines.push(`💳 Pago: ${info.payment}`);
    lines.push('⚠️ No incluye envío. El costo de envío se confirma según la zona.');
    if (info.delivery === 'Pagar al recibir') lines.push('🟠 Opción pagar al recibir disponible según cobertura y costo de envío.');
    if (info.note) lines.push(`📝 Nota: ${info.note}`);
    lines.push('');
    lines.push('📲 Consulta disponibilidad antes de confirmar.');
    return lines.join('\n');
  }
